...
def load_inventory_data(location_name):
    ...
    try:
        ...
        req_cols = ['Target_Price', 'Target_Grammage', 'Target_Width']
        for rc in req_cols:
            if rc not in df.columns:
                return pd.DataFrame(), f"Missing column {rc}."
        ...
        raw_price = df['Target_Price_Num'] / 2204.62
        df['Price/lb'] = raw_price.apply(lambda x: round(x + 1e-9, 2))
        
        width_in = df['Target_Width_Num'] / 25.4
        df['Paper Size (Inches)'] = width_in.round(1)
        
        weight_lbs = df['Target_Grammage_Num'] * 0.61386
        df['Paper Weight (#)'] = weight_lbs.round(1)
...
# The permanent 10% markup multiplier for paper handling
...
if not inventory_df.empty:
    ...
    if len(sizes) > 0:
        ...
        if len(weights) > 0:
            ...
            # Apply 10% markup and round purely to the penny for billing
            dynamic_paper_cost = round_cents(calculated_avg_price * markup_multiplier)
            web_width = selected_size
            basis_weight = selected_weight
            
            st.sidebar.success(f"Inventory Link Active: Billed at ${dynamic_paper_cost:.2f}/lb")
        else:
            inventory_loaded_successfully = False
    else:
        inventory_loaded_successfully = False
...
if not inventory_loaded_successfully:
    ...
    st.sidebar.info(f"Manual Rate: Billed at ${dynamic_paper_cost:.2f}/lb")
...
if entered_name == "boat hen":
    fancy_logo = "<div style='text-align: center; margin-top: 70px; opacity: 0.35;'><div style='font-family: \"Georgia\", serif; font-size: 34px; font-weight: normal; color: #2C3E50; letter-spacing: 5px;'>B <span style='font-style: italic; color: #E74C3C;'>&</span> H</div><div style='font-family: \"Helvetica Neue\", Helvetica, Arial, sans-serif; font-size: 9px; color: #7F8C8D; letter-spacing: 6px; text-transform: uppercase; margin-top: 6px; border-top: 1px solid #BDC3C7; padding-top: 6px; display: inline-block;'>Print Works</div></div>"
...
# 1. TOTAL PAGES CALCULATION (Exact match to Master Spreadsheet)
total_impressions = (press_run + waste_copies) * total_pages

# 2. PAPER YIELD AND WEIGHT
yield_factor = web_width * press_cutoff * basis_weight
pages_per_pound = 1900000 / yield_factor
total_pounds = total_impressions / pages_per_pound

# 3. FINAL PAPER COST (Rounded to the penny)
newsprint_cost = round_cents(total_pounds * dynamic_paper_cost)

# 4. INK COST based on ACTUAL total impressions (base run + waste)
...
b_col2.metric("Overhead", f"${overhead_cost:.