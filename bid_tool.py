...
waste_copies = st.sidebar.number_input("Waste Copies", value=750, step=50)
...
# This is your new 10% markup multiplier for paper handling
...
if not inventory_df.empty:
    ...
    if len(sizes) > 0:
        ...
        if len(weights) > 0:
            ...
            # Apply the markup right here before it hits the engine
            dynamic_paper_cost = round_cents(calculated_avg_price * markup_multiplier)
            
            web_width = selected_size
            basis_weight = selected_weight
            
            st.sidebar.success(f"Inventory Link Active: Raw ${calculated_avg_price:.2f}/lb → Billed at ${dynamic_paper_cost:.2f}/lb")
        else:
            inventory_loaded_successfully = False
    else:
        inventory_loaded_successfully = False
...
if not inventory_loaded_successfully:
    ...
    st.sidebar.info(f"Manual Price applied with 10% markup: ${dynamic_paper_cost:.2f}/lb")
