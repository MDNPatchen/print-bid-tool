...
def load_inventory_data(location_name):
    ...
    try:
        ...
        for i, line in enumerate(lines):
            if 'Ownership' in line or 'Grammage' in line:
                header_idx = i
                break
        ...
        # Check if we actually found the big three columns
        if 'Target_Price' not in df.columns or 'Target_Grammage' not in df.columns or 'Target_Width' not in df.columns:
            return pd.DataFrame(), f"Found '{target_file}', but couldn't locate Grammage, Width, or Price/mt columns inside it."
        ...
        # 4. Do the math
        df['Price/lb'] = (df['Target_Price'] / 2204.62).apply(lambda x: round_cents(x) if pd.notna(x) else x)
        df['Paper Size (Inches)'] = df['Target_Width'].apply(extract_num) / 25.4
        df['Paper Size (Inches)'] = df['Paper Size (Inches)'].round(2)
        
        df['Paper Weight (#)'] = df['Target_Grammage'].apply(extract_num) * 0.61386
        ...
        if df.empty:
            return df, f"Read '{target_file}' successfully, but couldn't extract valid numbers from the rows."
